//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace API.Models
{
    using System;
    
    public partial class AIPAvisosReporteGenerico_Result
    {
        public int CodigoAviso { get; set; }
        public string Titulo { get; set; }
        public string Descripcion { get; set; }
        public string ComentarioFinal { get; set; }
        public int CodigoProblema { get; set; }
        public Nullable<int> CodigoAvisoCampoValor { get; set; }
        public Nullable<int> CodigoCampo { get; set; }
        public string Valor { get; set; }
        public Nullable<int> Orden { get; set; }
        public Nullable<int> CodigoTipoCampo { get; set; }
        public string NombreProblema { get; set; }
        public string NombreCampo { get; set; }
        public string NombreTipoCampo { get; set; }
        public System.DateTime FechaInserto { get; set; }
    }
}
